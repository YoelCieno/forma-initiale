import { delay } from 'msw'
import { isValidTenant as isValidTenantBase } from '@repo/domain'
import { DEV_DELAY, PRODUCT_TENANT_CONFIGS } from '../constants'
import type { TenantId } from '@repo/domain'

const validTenantIds = Object.keys(PRODUCT_TENANT_CONFIGS) as TenantId[]
const isValidTenant = (value: string): value is TenantId => isValidTenantBase(value, validTenantIds)

const mockOkResponse = (data: unknown) => {
  return { ok: true, json: () => Promise.resolve(data) }
}

const delayDev = async (): Promise<void> => {
  if (import.meta.env.MODE === 'test') return
  await delay(DEV_DELAY)
}

export { isValidTenant, mockOkResponse, delayDev }
