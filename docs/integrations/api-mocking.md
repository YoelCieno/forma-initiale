...
### Integration test setup (web-vue)

Source: `apps/web-vue/vitest.setup.ts`
...
Source: `apps/web-vue/src/main.ts`
...
```bash
npx msw init apps/web-vue/public/ --save
```

This creates `apps/web-vue/public/mockServiceWorker.js`. The `--save` flag adds the path to `package.json` for future `msw init` calls.
...
- [`apps/web-vue/src/main.ts`](../../apps/web-vue/src/main.ts) — dev bootstrap loading MSW worker
- [`apps/web-vue/.env`](../../apps/web-vue/.env) — env flags for mocking
